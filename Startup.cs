...
using SocialDistancing.API.Common.Settings;
using SocialDistancing.API.Swagger;
using SocialDistancing.IoC.Configuration.DI;
...
namespace SocialDistancing.API
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            try
            {
                if (_appSettings.IsValid())
                {
                    ...
                    services.Configure<TerminalDatabaseSettings>(
                        Configuration.GetSection(nameof(TerminalDatabaseSettings)));

                    services.AddSingleton<ITerminalDatabaseSettings>(sp =>
                        sp.GetRequiredService<IOptions<TerminalDatabaseSettings>>().Value);
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IApiVersionDescriptionProvider provider, ILoggerFactory loggerFactory)
        {
            ...
            try
            {
                ...
                //HSH I commented this code because the reader don't have TLS support
                //app.UseHttpsRedirection();
                //HSH I'm not sure if we'll need that when we deploy to the production server
                app.UsePathBase("/SDS");
                app.UseRouting();
